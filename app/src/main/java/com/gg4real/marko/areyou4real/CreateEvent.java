...
import com.gg4real.marko.areyou4real.fragments.DatePickerFragment;
import com.gg4real.marko.areyou4real.fragments.TimePickerFragment;
...
public class CreateEvent extends AppCompatActivity implements AdapterView.OnItemSelectedListener, TimePickerDialog.OnTimeSetListener
    ...
    public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
        ...
        if((calendar.getTimeInMillis())<=(System.currentTimeMillis())){
            tvEventDate.setError("Molimo izaberite drugi datum");
            Toast.makeText(mContext, "Molimo izaberite valjan datum", Toast.LENGTH_SHORT).show();
            ...
            for (String reciverId : usersInGroup) {
                if (!reciverId.equals(FirebaseAuth.getInstance().getUid())) {
                    if (!duplicateCheck.contains(reciverId)) {
                        ...
                        final EventRequest eventRequest = new EventRequest(docId, "", selectedInteres, calendar.getTimeInMillis(), userName, userDocRef, reciverId, false,System.currentTimeMillis()+"");
    ...
    private boolean validateForm() {
        ...
        if (TextUtils.isEmpty(playersNeeded.getText().toString().trim())) {
